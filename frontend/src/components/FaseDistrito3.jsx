...
const FaseDistrito3 = ({ setFase }) => {
  ...
  useEffect(() => {
    const fetchNivel = async () => {
      try {
        const res = await fetch("https://datarunnersdeploy.onrender.com/niveles/6");
    ...
    const fetchTabla = async () => {
      try {
        const res = await fetch("https://datarunnersdeploy.onrender.com/niveles/tabla/6");
        const data = await res.json();
        setTabla(data.tabla);
      } catch (err) {
  ...
  const handleConsultar = async () => {
    try {
      const res = await fetch("https://datarunnersdeploy.onrender.com/niveles/6/resolver", {
        method: "POST",
      ...
      {(
        ...
        {nivelCompletado && (
          <button
            ...
            onClick={() => setFase("faseMercado1")} // Cambiar según el siguiente nivel
          >
            Siguiente Nivel ➡
          </button>
        )}
